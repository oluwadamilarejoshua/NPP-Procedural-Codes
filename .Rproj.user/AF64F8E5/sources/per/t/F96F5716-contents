...
for(i in c(1:length(Year_))){
  ...
  cassava_ <- (prod_to_use$Value[2] + crops_to_use[1, 2] - 
                 crops_to_use[1, 3] + variation_to_use$Value[2]) * 
...
Chad_NPPs <- t(NPP)[, -1]
Chad_NPPs <- row_to_names(Chad_NPPs, 1, remove_rows_above = T)
rownames(Chad_NPPs) <- Year_
