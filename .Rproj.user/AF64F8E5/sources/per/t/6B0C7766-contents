...
# Tajik_NPPs <- Arel_NPPs(country = "Tajikstan")
# Turkeman_NPPs <- Arel_NPPs(country = "Turkemanistan")
# Uzbek_NPPs <- Arel_NPPs(country = "Uzbekistan")


Turke_Uzbek <- function(country = "Turkmenistan"){
  ...
  for(i in c(1:length(Year_))){
    ...
    tomatoes_ <- (prod_to_use$Value[6] + crops_to_use[1, 16] - 
                    crops_to_use[1, 17] + variation_to_use$Value[6]) * 
    ...
    NPP_demand <- (NPP_food + NPP_feed + NPP_residues + NPP_fuel +
      NPP_burned) / popul_
...
Turkmen_NPP <- Turke_Uzbek()
Uzbek_NPP <- Turke_Uzbek(country = "Uzbekistan")
